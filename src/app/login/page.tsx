    ...
    <div className="min-h-screen flex items-center justify-center bg-black relative overflow-hidden selection:bg-indigo-500/30">
      ...
      <div className="w-full max-w-md mx-4 p-8 rounded-3xl border border-white/10 bg-zinc-950/50 backdrop-blur-xl shadow-2xl relative z-10 animate-fade-in-up">
        ...
        <form className="flex flex-col gap-4" action={signIn}>
          ...
          <SubmitButton className="w-full bg-white text-black hover:bg-zinc-200 font-semibold rounded-xl px-4 py-3 transition-all shadow-[0_0_20px_rgba(255,255,255,0.1)] hover:shadow-[0_0_30px_rgba(255,255,255,0.2)]" pendingText="Signing In...">
            Sign In
          </SubmitButton>
        ...
        <p className="text-center text-sm text-zinc-400 mt-8">
          ...
          <Link href={`/signup${nextUrl ? `?next=${encodeURIComponent(nextUrl)}` : ""}`} className="text-white hover:text-indigo-400 underline decoration-white/30 underline-offset-4 transition-colors font-medium">
            Sign Up
          </Link>
        </p>
      </div>
